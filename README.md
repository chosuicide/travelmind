...
我想尝试另一种做法：让模型负责理解意图、提出搜索参数和组织行程，让 Graph 负责阶段编排，让 Python 负责权限、状态、数据约束和最终写入。这样生成结果不只是一段文本，而是一份经过地点验证、能够在地图上展示、还能继续调整的行程。
...
TravelMind 使用 LangChain 描述工具，并用 LangGraph 编排行程生成状态。会话先从数据库读取 `collecting / preview_pending / generated / proposal_pending` 阶段，再让模型决定该阶段内的自然语言动作；行程生成则显式经过 `research → draft → validate → repair → complete`。模型不能选择跳过验证，也不能直接修改数据库。
...
flowchart LR
    ...
    P -->|用户确认| Q["生成任务"]
    Q --> W["后台 Worker"]
    W --> R["research · 查询真实 POI"]
    R --> T["高德地点与路线工具"]
    T --> L["draft · 组织行程"]
    L --> V["validate · 强制结构与质量验证"]
    V -->|需要修订| X["repair · 限定一次修复"]
    X --> V
    V -->|通过| D[("业务数据库")]
    M -->|用户确认| D
```
